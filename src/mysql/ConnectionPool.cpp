...
// C++11后静态局部变量在多线程环境下是线程安全的，因此可以保证线程安全性，可以以此方式实现单例模式
ConnectionPool* ConnectionPool::getConnectionPool(std::string filePath) {
    static ConnectionPool pool(filePath);
    return &pool;
}

ConnectionPool::ConnectionPool(std::string filePath) : filePath_(filePath) {
    // assert();
    parseJsonFile();
