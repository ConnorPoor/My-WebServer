...
void op1(int begin, int end) {
    for (int i = begin; i < end; ++i) {
        MysqlConn conn;
        conn.connect("normal", "normal", "test", "localhost");
        char sql[1024] = { 0 };
...
void test1() {
    ...
    ConnectionPool* pool = ConnectionPool::getConnectionPool("/home/ubuntu/project/My-Muduo/conf/MySQLconf.json");
    steady_clock::time_point begin = steady_clock::now();
    op2(pool, 0, 5000);
    steady_clock::time_point end = steady_clock::now();
    auto length = end - begin;
...
void test2() {
#if 1
    // 非连接池, 多单线程, 用时: 1633096999 纳秒, 1633 毫秒
    MysqlConn conn;
    conn.connect("normal", "normal", "test", "localhost");
    steady_clock::time_point begin = steady_clock::now();
...
#else
    // 连接池, 多单线程, 用时: 403887178 纳秒, 403 毫秒
    ConnectionPool* pool = ConnectionPool::getConnectionPool("/home/ubuntu/project/My-Muduo/conf/MySQLconf.json");
    steady_clock::time_point begin = steady_clock::now();
...
void test3() {
#if 0
    // 非连接池, 多单线程, 用时: 15912661273 纳秒, 15912 毫秒
    MysqlConn conn;
    conn.connect("normal", "normal", "test", "localhost");
    steady_clock::time_point begin = steady_clock::now();
